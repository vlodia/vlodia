...
import type { Vlodia } from '../core/vlodia';
import type { VlodiaConfig } from '@/types';
...
export class CLICommands {
  private orm: Vlodia | null = null;
  private config: VlodiaConfig | null = null;
  ...
  private async generateEntity(options: any): Promise<void> {
    ...
    try {
      ...
      spinner.succeed(chalk.green(`Entity generated successfully: ${outputPath}`));
    } catch (error) {
      spinner.fail(chalk.red(`Error generating entity: ${error}`));
      process.exit(1);
    }
  }
  ...
  private async validateSchema(): Promise<void> {
    ...
    try {
      ...
      spinner.fail(chalk.red(`Error validating schema: ${error}`));
      process.exit(1);
    }
  }
  ...
  private showExamples(): void {
    ...
    console.log(chalk.gray('     @PrimaryKey()'));
    console.log(chalk.gray('     id!: number;'));
    console.log(chalk.gray('     @Column()'));
    ...
    console.log(chalk.gray('   vlodia migration:run'));
    console.log(chalk.gray('   vlodia schema:sync'));
  }
   ...
   */
  private generateEntityTemplate(name: string, tableName?: string): string {
...
export class ${className} {
  ...
  private async initializeORM(): Promise<void> {
    ...
    // Dynamic import to avoid loading dependencies at module level
    const { Vlodia } = await import('../core/vlodia');
    this.orm = new Vlodia(this.config);
   ...
   */
  private generateMigrationTemplate(name: string, sql: string): string {
...
export class ${this.toPascalCase(name)} implements Migration {
  ...
  private async startRealtime(options: any): Promise<void> {
    ...
    try {
      ...
      spinner.fail(chalk.red(`Error starting real-time server: ${error}`));
      process.exit(1);
    }
  }
  ...
  private async generateGraphQL(options: any): Promise<void> {
    ...
    try {
      ...
      spinner.fail(chalk.red(`Error generating GraphQL schema: ${error}`));
      process.exit(1);
    }
  }
  ...
  private async visualizeSchema(options: any): Promise<void> {
    ...
    try {
      ...
      if (options.format === 'svg') {
        ...
        spinner.succeed(chalk.green(`Schema diagram generated: ${outputPath}.svg`));
      } else {
        const json = schemaDesigner.exportDiagram(diagram);
        await fs.writeFile(`${outputPath}.json`, json);
        spinner.succeed(chalk.green(`Schema diagram generated: ${outputPath}.json`));
      }
    } catch (error) {
      spinner.fail(chalk.red(`Error generating schema visualization: ${error}`));
      process.exit(1);
    }
  }
  ...
  private async initProject(options: any): Promise<void> {
    ...
    try {
      // Create vlodia directory for schema files
      ...
      // Create config.ts file in src/config directory
      await fs.ensureDir('src/config');
      const configContent = this.generateConfigFile(options);
      await fs.writeFile('src/config/vlodia.ts', configContent);
...
main();`;
  ...
  private async generateFromSchema(): Promise<void> {
    ...
    try {
      ...
      // Update config.ts in src/config directory
      await fs.ensureDir('src/config');
      await fs.writeFile('src/config/vlodia.ts', configContent);
      ...
      console.log(chalk.blue('\n📁 Generated files:'));
      console.log('├── src/config/vlodia.ts (updated)');
   ...
   */
  private parseSchemaFile(content: string): any {
   ...
   */
  private generateConfigFromSchema(config: any): string {
    return `/**
 * Vlodia Configuration
 * Auto-generated from schema.vlodia
 * Do not edit this file manually - use \`vlodia generate\` command
...
${config.entities.map((entity: any) => `import { ${entity.name} } from '../entities/${entity.name}';`).join('\n')}
...
export const config: VlodiaConfig = {
  ...
  entities: [${config.entities.map((entity: any) => entity.name).join(', ')}],
  logging: {
    level: 'info',
    format: 'text',
    queries: true
...
export default config;`;
   ...
   * Generate entities from schema
   */
  private async generateEntitiesFromSchema(config: any): Promise<void> {
