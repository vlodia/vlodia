...
export class QueryAnalyzer {
  ...
  private n1Patterns = new Map<string, N1Detection>();
   ...
   */
  private extractBasePattern(sql: string): string {
   ...
   */
  private generateOptimizationSuggestions(sql: string, n1Detected: boolean): string[] {
    ...
    for (const entity of entities) {
      if (sql.includes(entity.tableName)) {
        ...
        for (const relation of relations) {
          if (relation.type === 'OneToMany' || relation.type === 'ManyToMany') {
            suggestions.push(
              `Consider eager loading for ${entity.name}.${relation.propertyName} relation`
            );
            suggestions.push(
  ...
  private calculatePerformanceScore(executionTime: number, rowCount: number, n1Detected: boolean): number {
    ...
    if (n1Detected) score -= 40;

    // Penalize for large result sets without pagination
    if (rowCount > 1000) score -= 20;
    else if (rowCount > 100) score -= 10;
   ...
   */
  private updateN1Patterns(analysis: QueryAnalysis): void {
    if (!analysis.n1Detected) return;
