...
class SchemaDesigner {
     ...
     * Create column node
     */
    createColumnNode(entity, column) {
        return {
     ...
     * Create column edge
     */
    createColumnEdge(entityId, columnId, column) {
        return {
     ...
     */
    createRelationEdge(entity, relation) {
        const targetEntity = this.metadataRegistry.getEntity(relation.target);
        if (!targetEntity)
            return null;
        return {
     ...
     */
    applyHierarchicalLayout(nodes, _edges) {
    ...
    getEntityColor(entity) {
        ...
        if (relations.length > 5)
            return '#ff6b6b'; // Red for complex entities
        if (relations.length > 2)
            return '#4ecdc4'; // Teal for moderate complexity
    ...
    getColumnColor(column) {
        if (column.primary)
            return '#ff6b6b'; // Red for primary keys
        if (column.unique)
            return '#f9ca24'; // Yellow for unique columns
        if (column.nullable)
            return '#6c5ce7'; // Purple for nullable columns
...
//# sourceMappingURL=schema-designer.js.map